...
def load_input_into_list():
    """
    Takes our input file and pushes everything to a big 'ole list
    :return: The resultant list
    :rtype: iterator
    """

    _list = [cycle(line) for line in open("inputs/day3_input.txt", "r").read().splitlines()]
    for cyc in _list:
        next(cyc)
    itr = iter(_list)
    next(itr)
...
def get_answers(xslope, yslope):
    input_iter = load_input_into_list()
    ...
    while True:
        try:
            current_row = None
            current_column = None
            for __ in range(yslope):
                current_row = next(input_iter)
            for _ in range(x_slope):
                current_column = next(current_row)
            x_slope += xslope
            y_slope += yslope
            if current_column == '#':
                number_of_trees += 1
...
print(get_answers(3, 1))
print(prod((get_answers(1, 1), get_answers(3, 1), get_answers(5, 1), get_answers(7, 1), get_answers(1, 2))))