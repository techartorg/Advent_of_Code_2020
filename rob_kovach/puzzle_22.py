...
top and 1 on the bottom; player 2's deck also contains 5 cards, with 5 
...
input_ = open(__file__.replace('.py', '_input.txt')).read()

player1, player2 = input_.split('\n\n')

player1deck = [int(x) for x in player1.splitlines() if x.isnumeric()]
player2deck = [int(x) for x in player2.splitlines() if x.isnumeric()]
...
def play():
    p1 = Player(player1deck)
    p2 = Player(player2deck)

    while p1.deck_size > 0 and p2.deck_size > 0:
        card1 = p1.get_top_card()
        card2 = p2.get_top_card()
        if card1 > card2:
            p1.update_deck([card1, card2])
            p2.remove_card()
        else:
            p2.update_deck([card2, card1])
            p1.remove_card()
    
    deck = p1.deck if p1.deck_size > 0 else p2.deck
