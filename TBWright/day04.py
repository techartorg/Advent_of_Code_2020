...
def value_in_range(val, _min, _max):
    if _min <= int(val) <= _max:
        return True
    return False
...
def get_passport_dicts():
    passport_list = [line.replace('\n', ' ') for line in open("inputs/day4_input.txt", "r").read().split('\n\n')]
    passport_dicts = []
    ...
    for passport in passport_list:
        ...
        passport_dicts.append(passport_fields)
...
def get_part_01_answer():
    itr = 0
    for passport_dict in get_passport_dicts():
        dif = list(set(passport_dict.keys()).symmetric_difference(set(valid_fields)))
        if len(dif) == 0 or (len(dif) == 1 and dif[0] == 'cid'):
            itr += 1
    return itr
...
def get_part_02_answer():
    ...
    for passport_dict in get_passport_dicts():
        dif = list(set(passport_dict.keys()).symmetric_difference(set(valid_fields)))
        if len(dif) == 0 or (len(dif) == 1 and dif[0] == 'cid'):
            if (value_in_range(passport_dict['byr'], 1920, 2002) and len(passport_dict['byr']) == 4) and \
                   (value_in_range(passport_dict['iyr'], 2010, 2020) and len(passport_dict['iyr']) == 4) and \
