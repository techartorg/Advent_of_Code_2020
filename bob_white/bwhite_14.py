...
for line in pzl:
    ...
    if loc == "mask":
        ...
        for idx, v in enumerate(mask):
            if v == "1":
                addrs[:] = [[av if i != idx else "1" for i, av in enumerate(addr)] for addr in addrs]
            elif v == "X":
                addrs[:] = [[av if i != idx else nv for i, av in enumerate(addr)] for addr in addrs for nv in ("0", "1")]
        for addr in addrs:
